...
class Student{
	...
	public function insertStudent($name, $roll){
		...
		if (empty($name) || empty($roll)) {
			...
			$stu_query = "INSERT INTO student_data(name, roll) VALUES('$name', '$roll')";
	...
	public function getAllData($dt){
		...
		$query = "SELECT student_data.u_f_name, tbl_attendance.*
				FROM student_data
				INNER JOIN tbl_attendance
				ON student_data.roll = tbl_attendance.roll
				WHERE att_time = '$date'";
		$result = $this->db->select($query);
		return $result;
	}
